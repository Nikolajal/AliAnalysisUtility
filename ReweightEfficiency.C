...
void isSameBinning(TH1F* h0,TH1F* h1){
  ...
  for(j=1;j<=h0->GetNbinsX();j++){
    A_0=h0->GetXaxis()->GetBinLowEdge(j);
    B_0=h0->GetXaxis()->GetBinLowEdge(j+1);
    A_1=h1->GetXaxis()->GetBinLowEdge(j);
    B_1=h1->GetXaxis()->GetBinLowEdge(j+1);

    if(fabs(A_0-A_1)<1.e-6 && fabs(B_0-B_1)<1.e-6) continue;
    else{
      cerr<<"Error in ReweightEfficiency(): mismatched bins detected for histograms "<<h0->GetName()<<" and "<<h1->GetName()<<endl;
      break;
    }
