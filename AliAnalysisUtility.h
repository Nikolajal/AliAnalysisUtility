...
//--------------------------------//
//
TString                 kMSG_PrintTimer             =   "\r[INFO] [%s] Event # %7.f %s | %3.1f %% | %7.2f %s events/s | Time: %02.0f:%02.0f:%02.0f | ETA: %02.0f:%02.0f:%02.0f";
//
void
fStartTimer
 ( TString fTimerName )    {
...
fPrintLoopTimer
 ( TString fTimerName, Int_t iEvent, Int_t nEntries, Int_t iPrintInterval )   {
    ...
    // Printing
    cout << "\33[2K" << flush;
    cout << Form(kMSG_PrintTimer.Data(), uCurrentDateTime().Data(),  fShowPrintEvnt,  fSuffix.Data(), 100.*fProcessedFrac, fShowSpeedEvnt,  fSuffix.Data(), fShowElapsedHor, fShowElapsedMin,  fShowElapsedSec, fShowEstimatHor, fShowEstimatMin, fShowEstimatSec) << flush;
}
//
//------------------------------//
